                    ...
                    try:
                        if not is_df.empty and not bs_df.empty:
                            ...
                            if is_regular: # Doanh nghiệp thường
                                ...
                                fig1 = go.Figure()
                                fig1.add_trace(go.Bar(x=is_df['Period'], y=rev_bn, name=t("Doanh thu"), marker_color='#29B6F6'))
                                fig1.add_trace(go.Bar(x=is_df['Period'], y=profit_bn, name=t("Lợi nhuận ròng"), marker_color='#69F0AE'))
                                sales_clean = rev_bn.replace(0, 1)
                                margin = (profit_bn / sales_clean) * 100
                                fig1.add_trace(go.Scatter(x=is_df['Period'], y=margin, name=t("Biên LN (%)"), yaxis="y2", line=dict(color='#FFD54F', width=2), mode='lines+markers'))
                                fig1.update_layout(title=t("Hiệu suất (Tỷ VNĐ)"), barmode='group', template='plotly_dark', showlegend=True, legend=dict(orientation="h", y=-0.2), yaxis2=dict(title="%", overlaying="y", side="right", showgrid=False))
                                ...
                                fig2.update_layout(title=f"{t('Kết quả kinh doanh')} ({year_latest})", template='plotly_dark')
                                ...
                                fig3 = go.Figure()
                                fig3.add_trace(go.Bar(x=bs_df['Period'], y=liab_bn, name=t("Nợ phải trả"), marker_color='#00BCD4'))
                                fig3.add_trace(go.Bar(x=bs_df['Period'], y=eq_bn, name=t("Vốn CSH"), marker_color='#4DB6AC'))
                                eq_clean = eq_bn.replace(0, 1)
                                debt_eq = (liab_bn / eq_clean) * 100
                                fig3.add_trace(go.Scatter(x=bs_df['Period'], y=debt_eq, name=t("Nợ/VCSH (%)"), yaxis="y2", line=dict(color='#FFEE58', width=2), mode='lines+markers'))
                                fig3.update_layout(title=t("Tài sản và Vốn chủ sở hữu (Tỷ VNĐ)"), barmode='group', template='plotly_dark', legend=dict(orientation="h", y=-0.2), yaxis2=dict(title="%", overlaying="y", side="right", showgrid=False))
                                ...
                                fig4.update_layout(title=f"{t('Vị thế tài chính')} ({bs_latest_yr})", barmode='group', template='plotly_dark', legend=dict(orientation="h", y=-0.2), yaxis_title="Tỷ VNĐ")
                                ...
                                fig1 = go.Figure()
                                fig1.add_trace(go.Bar(x=is_df['Period'], y=rev_bn, name=t("Tổng Thu HĐ"), marker_color='#29B6F6'))
                                fig1.add_trace(go.Bar(x=is_df['Period'], y=profit_bn, name=t("Lợi nhuận ròng"), marker_color='#69F0AE'))
                                rev_clean = rev_bn.replace(0, 1)
                                margin = (profit_bn / rev_clean) * 100
                                fig1.add_trace(go.Scatter(x=is_df['Period'], y=margin, name=t("Biên LN (%)"), yaxis="y2", line=dict(color='#FFD54F', width=2), mode='lines+markers'))
                                fig1.update_layout(title=t("Hiệu suất Ngân hàng (Tỷ VNĐ)"), barmode='group', template='plotly_dark', showlegend=True, legend=dict(orientation="h", y=-0.2), yaxis2=dict(title="%", overlaying="y", side="right", showgrid=False))
                                col1.plotly_chart(fig1, width='stretch')
                                ...
                                fig3.update_layout(title=t("Huy động vốn vs Cho vay (Tỷ VNĐ)"), barmode='group', template='plotly_dark', legend=dict(orientation="h", y=-0.2))
                                col1.plotly_chart(fig3, width='stretch')
            ...
            try:
                ...
                if vnindex_data and not vnindex_data['Price'].empty:
                    ...
                    if not corr_df.empty and len(corr_df) > 10:
                        ...
                        fig_corr.update_layout(height=400, title=t("So sánh Hiệu suất (Đã chuẩn hóa về 100 điểm)"), yaxis_title=t("Tăng trưởng (%)"), template="plotly_dark")
